#!/usr/bin/env python
# -*- coding: utf-8 -*-
# build exe cmd:  pyinstaller --onefile labelImg.py
import argparse
import codecs
...
op = open  # open function has been override, so use op
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, custom_setting_file=None, default_save_dir=None):
        ...
        # Load custom setting file to dict
        assert os.path.exists(custom_setting_file), "there is not custom_setting file in {}".format(custom_setting_file)
        with op(custom_setting_file, "r") as jf:
            self.custom_setting = json.load(jf)
        print(self.custom_setting)
        ...
        change_save_dir = action(
            get_str("changeSaveDir"), self.change_save_dir_dialog, "Ctrl+r", "open", get_str("changeSavedAnnotationDir")
        )

        open_annotation = action(
            get_str("openAnnotation"), self.open_annotation_dialog, "Ctrl+Shift+O", "open", get_str("openAnnotationDetail")
        )
        copy_prev_bounding = action(
            get_str("copyPrevBounding"), self.copy_previous_bounding_boxes, "Ctrl+v", "copy", get_str("copyPrevBounding")
        )
        ...
        help_default = action(
            get_str("tutorialDefault"), self.show_default_tutorial_dialog, None, "help", get_str("tutorialDetail")
        )
        ...
        zoom_in = action(
            get_str("zoomin"), partial(self.add_zoom, 10), "Ctrl++", "zoom-in", get_str("zoominDetail"), enabled=False
        )
        zoom_out = action(
            get_str("zoomout"), partial(self.add_zoom, -10), "Ctrl+-", "zoom-out", get_str("zoomoutDetail"), enabled=False
        )
        zoom_org = action(
            get_str("originalsize"), partial(self.set_zoom, 100), "Ctrl+=", "zoom", get_str("originalsizeDetail"), enabled=False
        )
        fit_window = action(
            get_str("fitWin"), self.set_fit_window, "Ctrl+F", "fit-window", get_str("fitWinDetail"), checkable=True, enabled=False
        )
        fit_width = action(
            get_str("fitWidth"),
            self.set_fit_width,
    ...
    def edit_label(self):
        ...
        if text is not None:
            item.setText(text)
            item.setBackground(generate_color_by_text(text, self.custom_setting))
            self.set_dirty()
    ...
    def load_labels(self, shapes):
        ...
        for label, points, line_color, fill_color, difficult in shapes:
            ...
            if line_color:
                ...
                shape.line_color = generate_color_by_text(label, self.custom_setting)

            if fill_color:
                shape.fill_color = QColor(*fill_color)
            else:
                shape.fill_color = generate_color_by_text(label, self.custom_setting)
    ...
    def label_item_changed(self, item):
        ...
        if label != shape.label:
            ...
            shape.line_color = generate_color_by_text(shape.label, self.custom_setting)
            self.set_dirty()
        else:  # User probably changed item visibility
            self.canvas.set_shape_visible(shape, item.checkState() == Qt.Checked)

    def getColorFromLabelText(self):

        if not self.use_default_label_checkbox.isChecked() or not self.default_label_text_line.text():
            if self.single_class_mode.isChecked() and self.lastLabel:
                text = self.lastLabel
                return generate_color_by_text(text, self.custom_setting)
            ...
            if LabelFile.is_label_file(unicode_file_path):
                try:
                    ...
                    self.error_message(
                        ...
                        (u"<p><b>%s</b></p>" u"<p>Make sure <i>%s</i> is a valid label file.") % (e, unicode_file_path),
                    )
...
def get_main_app(argv=[]):
    ...
    argparser.add_argument(
        "class_file", default=os.path.join(os.path.dirname(__file__), "data", "predefined_classes.txt"), nargs="?"
    )
    argparser.add_argument(
        "custom_setting_file", default=os.path.join(os.path.dirname(__file__), "data", "custom_setting.json"), nargs="?"
    )
    ...
    args.custom_setting_file = args.custom_setting_file and os.path.normpath(args.custom_setting_file)
    args.save_dir = args.save_dir and os.path.normpath(args.save_dir)

    # Usage : labelImg.py image classFile saveDir
    win = MainWindow(args.image_dir, args.class_file, args.custom_setting_file, args.save_dir)
    win.show()
    return app, win
